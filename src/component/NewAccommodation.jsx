...
console.log(filesArray?.length)
  ...
  for (let i = 0; i < filesArray?.length; i++) {
   ...
   axios.post('/uploads', data ).then(response => {
...
async function savePlace(ev) {
 ...
 if(link){
// update
await axios.put('/places',  {...placeData , link} );
...
function moveToFirst(filename) {
  ...
  return (
    ...
    <div className={` duration-300  ${ !add ?   'top-[50%]' : 'top-[-100%] ' }  flex  z-50 fixed h-full w-full relateve `}>
    <div onClick={()=>setAdd(!add)} className={`  flex  fixed h-full w-full absulet ${ !add ? ' opacity-50 top-0  ' : 'opacity-0  top-[-100%]   '  }  right-0 bg-black `} ></div>
...
<h1>Title</h1>
        <p className=' text-gray-500 text-xs'>Title for your place should be short and catchy as in advertisment</p>
          <input className='  h-12  w-full rounded-lg px-3 bg-slate-100' type="text" placeholder=' Title ' value={title} onChange={ (e)=> setTitle(e.target.value) } />
       
        <h1>Address</h1>
<p className=' text-gray-500 text-xs'>Address to your place</p>
          <input className='  h-12 w-full rounded-lg px-3 bg-slate-100' type="text" placeholder=' Address '  value={address} onChange={ (e)=> setAddress(e.target.value) } />
        <h1>Photos</h1>
<p className=' text-gray-500 text-xs'>More Photos More  batter</p>
         <div className=' flex flex-row gap-2'>
         <input value={photolink} onChange={ (e)=> setPhotolink(e.target.value) } className=' h-12  w-full rounded-lg px-3 bg-slate-100' type="text" placeholder=' Add using link to your photo .... ' />
         <button onClick={sendLinke} className='   font-medium   justify-center flex flex-row items-center gap-2  w-[15%]  rounded-lg  text-xs md:text-sm bg-gray-200'>Add Photo
...
<button onClick={ ()=> deletphoto(pic) } ><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
</svg>
</button>
<button   onClick={ ()=> moveToFirst(pic) } ><svg xmlns="http://www.w3.org/2000/svg" fill={`   ${ addedPhotos.indexOf(pic) === 0 ?   ' ' : ' none'   }  `} viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
</svg>
</button>
</div>
     <img className='  object-cover h-28 w-72 rounded-lg ' src={` http://localhost:4000/uploads/${pic}  `} alt="" />
          ...
          <label   className=' cursor-pointer p-4 py-6 rounded-xl border-2 border-solid flex justify-center items-center'  htmlFor='fileup'> <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">

 
  <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
</svg>
<input type="file" className=' hidden z-20 ' onChange={uploadphoto} name="" id='fileup' multiple  />
</label>
...
<p className=' text-gray-500 text-xs'>Descripton of the place</p>
 <textarea value={description} onChange={ (e)=> setDescription(e.target.value) } className=' border-2 border-solid rounded-xl w-full h-[100px]'/>
...
<div>
  ...
  <input   value={checkIn} onChange={ (e)=> setCheckIn(e.target.value) } type="text" className=' w-full h-12  border-solid border-2 rounded-lg' />
</div>
<div>
  <h3 className=' text-[9px] md:text-lg mt-3'>Check out time</h3>
  <input value={checkOut} onChange={ (e)=> setCheckOut(e.target.value) } type="text" className=' w-full h-12  border-solid border-2 rounded-lg' />
</div>
<div>
  <h3 className=' text-[9px] md:text-lg mt-3'> Max number of guests</h3>
  <input value={maxGuests} onChange={ (e)=> setMaxGuests(e.target.value) } type="text" className=' w-full h-12  border-solid border-2 rounded-lg' />
</div>

<div>
  <h3 className=' text-[9px] md:text-lg mt-3'> Price per night </h3>
  <input value={price} onChange={ (e)=> setPrice(e.target.value) } type="number" className=' w-full h-12  border-solid border-2 rounded-lg' />
</div>
 ...
 <div className=' w-full flex justify-center items-center'>
  <button    className='w-[90%] h-12 bg-gray-300  rounded-lg ' >  {link  ? 'Save the Changes' : 'Save'     }  </button>
 </div>
      </form>
