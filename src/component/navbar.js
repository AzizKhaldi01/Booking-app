...
function Navbar() {
  ...
  const handleLogout = () => {
    Logout(); // You can call the Logout function here
    setUser(null);
    localStorage.clear();
  ...
  return (
    <div className=" hidden md:flex  flex-col w-full">
      <div className=" w-full h-16  items-center  z-50 border-b-[1px]  border-solid bg-white  fixed top-0 right-0 flex flex-row justify-between  px-12 p-10  ">
        ...
        <div className=" text-[.9rem] text-gray-500  shadow-sm hover:shadow-md duration-200  cursor-pointer h-[3rem] w-[27%]    flex flex-row items-center justify-between pl-3 pr-1  z-20  py-3 rounded-full  border-solid border-[1px] ">
          <h>Anywhere</h>
        ...
        <span className=" text-white bg-main p-2 rounded-[50%] ">
        ...
        <div
          onClick={() => setTab(!Tab)}
          className=" hover:shadow-lg duration-200  cursor-pointer p-2 px-3 rounded-full border-solid border-[1px] relative  flex flex-row  justify-center items-center  gap-3"
        >
