...
function Gestes({ title, desc , Geust ,   onClick , dicrementGuests, maxGuests , mobile}) {
 
 

  let max =      title === "Pets" && Geust >= 2 || Geust >= maxGuests || title === "Infants" && Geust >= 3   ||  Geust?.Children + Geust?.Adults  >= maxGuests  
    ...
    <div className=" flex flex-row w-full     ">
      ...
      <div  className=" gap-2 flex  items-center     ">
        ...
        <input
          ...
          value={Geust}
        />
         <button     disabled={max}      onClick={(e)=>  onClick(e , title)} className={ `  ${title === "Pets" && Geust >= 2 || Geust >= maxGuests || title === "Infants" && Geust >= 3   ||  Geust?.Children + Geust?.Adults  >= maxGuests   ?  '  cursor-default border-gray-200 text-gray-200 hover:bg-white  hover:text-gray-200 active:scale-100 ' : ''    }   rounded-[50%] h-7 hover:bg-[#578280]  hover:text-white  active:scale-95 hover:bg-   text-[#578280] w-7 border-[1px]   border-[#578280] `}>
          +
        </button>
      </div>
    </div>
