...
function PlaceDetailes() {
  ...
  return (
    <div className=" px-10 w-full h-full  gap-12 mt-32 justify-between flex flex-row">
      <div className=" w-[40%] flex flex-col     ">
        <div className=" text-3xl font-bold max-w-[80%] ">{data.title}</div>
        <p className="  text-xs py-3 text-gray-600">{data.address}</p>
        ...
        <div className=" cursor-pointer    flex-row    justify-between    text-sm items-center relative w-[70%] h-20 border-2 border-solid rounded-full my-10 flex ">
        ...
        <p  className=" w-full  leading-6    text-gray-800 text-[13px] py-3  ">
          {data.description.length >= 420
            ? <p  > {data.description.substring(0, 420) + "..." }     <p onClick={ ()=> setExtra(!extra)  } className=" hover:opacity-70 underline cursor-pointer flex items-center    w-[30%] text-black">  Show More <ChevronRightIcon/>  </p> </p>
            : <p> {data.description}   <p onClick={ ()=> setExtra(!extra)  } className=" hover:opacity-70 underline cursor-pointer flex items-center    w-[30%] text-black">  Show More <ChevronRightIcon/>  </p>  </p>
 }
            
        </p>

<div   className={ `  duration-300 ease-out      overflow-hidden    flex flex-col items-center  bg-white fixed h-full  ${ morephotos ? ' top-0 opacity-[100%] ' : ' opacity-0 top-[100%]'} w-full right-0    z-50  `}>
<div className="  p-3 w-full absolute h-16 bg-white"> <span onClick={hendelmorephotos} className=" cursor-pointer"><CloseIcon  fontSize="small"/> </span> </div>
 
 <div className=  {` ${ !morephotos ? '  opacity-0  ' : ' opacity-100  '}   duration-1000  flex flex-col items-center      h-full  w-full    overflow-auto   `} >

   {data.photos.map((photo)=>(
     <img  className=" w-[60%]   " src={"http://localhost:4000/uploads/"+photo} alt="" />
  )) }
 </div>
      ...
      <div onClick={hendelmorephotos} className=" w-full  gap-10 md:w-[60%]  cursor-pointer ">
        <img
